...
struct command<Out> {
    ...
    using resumption_type = Answer(Out);
};
...
typename H::answer_type handle_ref(
    int64_t label, std::function<typename H::body_type(handler_ref)> body,
    Args&&... args);
...
resumption<typename H::answer_type(A)> wrap(
    int64_t label, std::function<typename H::body_type(A)> body,
    Args&&... args);
...
resumption<typename H::answer_type()> wrap_with(
    int64_t label, std::function<typename H::body_type()> body,
    std::shared_ptr<H> handler);
...
typename Cmd::out_type command_clause<Answer, Cmd>::invoke_command(
    ...
    std::move(metastack.front()->fiber)
        .resume_with([&](ctx::fiber&& prev) -> ctx::fiber {
            ...
            // Keep the handler alive for the duration of the command clause
            // call
...
class resumption_base {
    ...
    friend typename H::answer_type handle_with(
        ...
        std::shared_ptr<H> handler);
    template <typename>
    friend class resumption;
    template <typename, typename>
    friend class resumption_data;
...
class handler : public cpp_effects_internals::metaframe,
   ...
   public:
    using cpp_effects_internals::command_clause<Answer,
                                                Cmds>::handle_command...;
    using cpp_effects_internals::command_clause<Answer,
                                                Cmds>::invoke_command...;
   ...
   public:
    ...
    virtual void debug_print() const override {
        std::cout << cpp_effects_internals::metaframe::label << ":"
                  << typeid(*this).name();
...
typename H::answer_type handle_ref(
    ...
    Args&&... args) {
    if constexpr (!std::is_void<typename H::answer_type>::value) {
        return handle_with_ref(
            label, body, std::make_shared<H>(std::forward<Args>(args)...));
    } else {
...
resumption<typename H::answer_type()> wrap(
    ...
    return resumption<typename H::answer_type()>([=](A a) {
        return handle<H>(label, std::bind(body, a),
...
typename Cmd::out_type invoke_command(handler_ref it, const Cmd& cmd) {
    ...
    std::cerr << "error: selected handler does not handle "
              << typeid(Cmd).name() << std::endl;
    debug_print_metastack();
    exit(-1);
}
...
typename Cmd::out_type static_invoke_command(int64_t goto_handler,
    ...
    auto it = std::find_if(metastack.begin(), metastack.end(), cond);
    if (it != metastack.end()) {
        return (static_cast<H*>(it->get()))
            ->H::invoke_command(std::next(it), cmd);
    }
    std::cerr << "error: handler with id " << goto_handler
              << " does not handle " << typeid(Cmd).name() << std::endl;
    debug_print_metastack();
...
handler_ref find_handler() {
    ...
    std::cerr << "error: cpp_effects::find_handler did not find a handler"
              << std::endl;
    debug_print_metastack();
    exit(-1);
}
