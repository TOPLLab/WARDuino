...
export class WARDuinoDebugSession extends LoggingDebugSession {
    ...
    protected async launchRequest(response: DebugProtocol.LaunchResponse, args: any) {
		...
		let compiler : WASMCompilerBridge = new WASMCompilerBridge(process.cwd(),args.program);
		compiler.CompileWast();

		let sourceMap  =  await compiler.CompileWast().catch( (reason) => this.handleCompileError(reason) );
		if(sourceMap) {
	...
	private setLineNumberFromPC(pc:number) {
		this.sourceMap?.forEach( (info) => {
			...
			if( Math.abs(pc-address) == 0) {
				this.test_current_line = info.lineInfo.line -1;
			}
		});
	}
    ...
    protected stackTraceRequest(response: DebugProtocol.StackTraceResponse, args: DebugProtocol.StackTraceArguments): void {
		...
		const sf: DebugProtocol.StackFrame = new StackFrame(0, 
				...
				this.convertDebuggerLineToClient(this.test_current_line));
