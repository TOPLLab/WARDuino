...
struct SerializeData *ProxySupervisor::serializeRFC(RFC *callee) {
    ...
    return nullptr;
    //    const unsigned short serializationSize =
    //    sizeSerializationRFC(callee->type); auto *buffer = new unsigned
    //    char[serializationSize];
    //
    ...
    //    hexa[hexa_size] = '\n';
    //    hexa[hexa_size + 1] = '\0';  // TODO remove zero termination and +2
    //    above
    //
    //    delete[] buffer;
...
bool ProxySupervisor::call(RFC *callee) {
    ...
    //    cmdBufferLen); this->deserializeRFCResult(callee); return true;
}
