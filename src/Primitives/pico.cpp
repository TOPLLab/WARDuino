...
def_prim(chip_pin_mode, twoToNoneU32) {
    gpio_init(arg1.uint32);
    // 2 is output, 0 is input. Why not just 1 and 0?
    gpio_set_dir(arg1.uint32, arg0.uint32 == 2);
    pop_args(2);
    return true;
}

def_prim(chip_digital_write, twoToNoneU32) {
    gpio_put(arg1.uint32, arg0.uint32);
    pop_args(2);
    return true;
}

def_prim(chip_delay, oneToNoneU32) {
    sleep_ms(arg0.uint32);
    pop_args(1);
    return true;
}
