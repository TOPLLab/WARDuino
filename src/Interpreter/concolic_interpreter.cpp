...
Block *ConcolicInterpreter::pop_block(Module *m) {
    ...
    if (t->result_count == 1) {
        ...
        if (frame->sp < ectx->sp) {
            ...
            m->symbolic_stack[frame->sp + 1] = m->symbolic_stack[ectx->sp];
            ectx->sp = frame->sp + 1;
        }
    } else {
        if (frame->sp < ectx->sp) {
...
void ConcolicInterpreter::setup_call(Module *m, uint32_t fidx) {
    ...
    for (uint32_t lidx = 0; lidx < func->local_count; lidx++) {
        ...
        m->symbolic_stack[ectx->sp] = encode_as_symbolic(m, &ectx->stack[ectx->sp]);
    }
...
void ConcolicInterpreter::load(Module *m, uint32_t offset, uint32_t addr,
    ...
    m->symbolic_stack[ectx->sp] = z3::concat(expressions).simplify();
    ...
    if (size < full_size) {
        if (sign_extend) {
            m->symbolic_stack[ectx->sp] = sext(m->symbolic_stack[ectx->sp].value(),
                                            full_size * 8 - size * 8);
        } else {
            m->symbolic_stack[ectx->sp] = zext(m->symbolic_stack[ectx->sp].value(),
                                            full_size * 8 - size * 8);
        }
    }
}
...
bool ConcolicInterpreter::i_instr_if(Module *m, uint8_t *block_ptr) {
    ...
    z3::expr sym_cond = m->symbolic_stack[ectx->sp].value() != 0;
...
bool ConcolicInterpreter::i_instr_br_if(Module *m) {
    ExecutionContext *ectx = m->warduino->execution_context;
    uint32_t cond = ectx->stack[ectx->sp].value.uint32;
    z3::expr sym_cond = z3::ite(m->symbolic_stack[ectx->sp].value() != 0,
...
bool ConcolicInterpreter::i_instr_select(Module *m) {
    ...
    m->symbolic_stack[ectx->sp] =
        z3::ite(sym_cond != 0, m->symbolic_stack[ectx->sp].value(),
                m->symbolic_stack[ectx->sp + 1].value());
...
bool ConcolicInterpreter::i_instr_get_local(Module *m) {
    ...
    m->symbolic_stack[ectx->sp] = m->symbolic_stack[ectx->fp + arg];
    return true;
}
...
bool ConcolicInterpreter::i_instr_set_local(Module *m) {
    ...
    // complicated.
    int32_t arg = read_LEB_32(&ectx->pc_ptr);
    m->symbolic_stack[ectx->fp + arg] = m->symbolic_stack[ectx->sp];
...
bool ConcolicInterpreter::i_instr_tee_local(Module *m) {
    ...
    // m->symbolic_stack[ectx->sp].value() << std::endl;
    m->symbolic_stack[ectx->fp + arg] = m->symbolic_stack[ectx->sp];
#if TRACE
    debug("      - arg: 0x%x, to %s\n", arg, value_repr(&ectx->stack[ectx->sp]));
#endif
    return true;
}
...
bool ConcolicInterpreter::i_instr_get_global(Module *m) {
    ...
    int32_t arg = read_LEB_32(&ectx->pc_ptr);
#if TRACE
    debug("      - arg: 0x%x, got %s\n", arg, value_repr(m->globals[arg]->value));
#endif
    ectx->stack[++ectx->sp] = *m->globals[arg]->value;
    m->symbolic_stack[ectx->sp] = m->symbolic_globals[arg];
    return true;
}
...
bool ConcolicInterpreter::i_instr_set_global(Module *m) {
    ...
    m->symbolic_globals[arg] = m->symbolic_stack[ectx->sp].value();
    *m->globals[arg]->value = ectx->stack[ectx->sp--];
#if TRACE
    debug("      - arg: 0x%x, got %s\n", arg, value_repr(&ectx->stack[ectx->sp + 1]));
#endif
    return true;
}
...
bool ConcolicInterpreter::i_instr_current_memory(Module *m) {
    ...
    m->symbolic_stack[ectx->sp] = m->symbolic_memory.symbolic_pages;
    return true;
}
...
bool ConcolicInterpreter::i_instr_unary_u32(Module *m, uint8_t opcode) {
    ...
    m->symbolic_stack[ectx->sp] = ite(m->symbolic_stack[ectx->sp].value() == 0,
...
bool ConcolicInterpreter::i_instr_math_u32(Module *m, uint8_t opcode) {
    ...
    z3::expr a = m->symbolic_stack[ectx->sp - 1].value();
    z3::expr b = m->symbolic_stack[ectx->sp].value();
...
bool ConcolicInterpreter::i_instr_math_u64(Module *m, uint8_t opcode) {
    ...
    z3::expr d = m->symbolic_stack[ectx->sp - 1].value();
    z3::expr e = m->symbolic_stack[ectx->sp].value();
    std::optional<z3::expr> c;
    ectx->sp -= 1;
    switch (opcode) {
        case 0x51:
...
bool ConcolicInterpreter::i_instr_unary_i32(Module *m, uint8_t opcode) {
    ExecutionContext *ectx = m->warduino->execution_context;
    z3::expr e = m->symbolic_stack[ectx->sp].value().simplify();
    switch (opcode) {
        case 0x67: { // i32.clz
            ...
            m->symbolic_stack[ectx->sp] = result;
            break;
        }
        case 0x68: { // i32.ctz
            z3::expr result =  m->ctx.bv_val(32, 32);
...
bool ConcolicInterpreter::i_instr_unary_floating(Module *m, uint8_t opcode) {
    ...
    switch (opcode) {
        // unary f32
        case 0x8b:
            ectx->stack[ectx->sp].value.f32 = fabs(ectx->stack[ectx->sp].value.f32);
            m->symbolic_stack[ectx->sp] =
                z3::abs(m->symbolic_stack[ectx->sp].value());
            break;  // f32.abs
        case 0x8c:
            // TODO: Symbolic semantics
            assert(false);
...
bool ConcolicInterpreter::i_instr_conversion(Module *m, uint8_t opcode) {
    ...
    switch (opcode) {
        case 0xa7:
            m->symbolic_stack[ectx->sp] = m->symbolic_stack[ectx->sp]->extract(31, 0);
            break;  // i32.wrap/i64
        case 0xa8:
            m->symbolic_stack[ectx->sp] = z3::fpa_to_sbv(m->symbolic_stack[ectx->sp].value(), 32);
            break;  // i32.trunc_s/f32
        case 0xa9:
            m->symbolic_stack[ectx->sp] = z3::fpa_to_ubv(m->symbolic_stack[ectx->sp].value(), 32);
        ...
        case 0xac:
            m->symbolic_stack[ectx->sp] =
                sext(m->symbolic_stack[ectx->sp].value(), 32);
            break;  // i64.extend_s/i32
        case 0xad:
            m->symbolic_stack[ectx->sp] =
                zext(m->symbolic_stack[ectx->sp].value(), 32);
        ...
        case 0xb2:
            ...
            //m->symbolic_stack[ectx->sp].value().mk_from_ieee_bv(m->ctx.fpa_sort(8, 24));
            m->symbolic_stack[ectx->sp] = sbv_to_fpa(m->symbolic_stack[ectx->sp].value(), z3::to_sort(m->ctx, Z3_mk_fpa_sort_single(m->ctx)));
            //m->symbolic_stack[ectx->sp] = sbv_to_fpa(m->symbolic_stack[ectx->sp].value(), m->ctx.fpa_sort(8, 24));
